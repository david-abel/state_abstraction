...
\thm{Lemma}{1}{Given two MDPs, $M_1$ and $M_2$ which differ only by $T$ and $R$, if $\forall_{s,a} |T_1(s,a,\cdot)-T_2(s,a,\cdot)| \leq 2\beta$ and $\forall_{s,a} |R_1(s,a)-R_2(s,a)|\leq \alpha$ then for some fixed policy $\pi$ then $\forall_{s,a} |Q_1^\pi(s,a)-Q_2^\pi(s,a)| \leq \frac{\alpha v_{max}}{C(1-\gamma)} = \epsilon$ (Simulation Lemma)}
...
\thm{Lemma}{3}{Under the optimal Q function approximate abstraction, $\forall_s : |Q_A^*(s,.) - Q_G^*(s,.)| \leq \epsilon$ }
