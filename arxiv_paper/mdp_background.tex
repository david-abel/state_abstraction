...
\end{equation}
We let \textsc{RMax} denote the maximum reward (which is 1), and \textsc{QMax} denote the maximum $Q$ value, which is $\frac{\textsc{RMax}}{1-\gamma}$. The value function, $V$, defined under a given policy, denoted $V^\pi(s)$, is defined as:
\begin{equation}
V^\pi(s) = Q^\pi(s,\pi(s)).
\end{equation}
