...
We are interested in how the optimal policy in the abstract \ac{MDP} performs in the ground \ac{MDP}. 

\bdefn{$\pi_A^*$}
To evaluate a the optimal abstractpolicy $\pi_A:\mathcal{A}\rightarrow\mathcal{A}$, we map a ground state into the abstract state space, and query the abstract policy for behavior. Specifically:
\begin{equation}
...
\bdefn{$Q_G$, $V_G$}
Let $Q_G: \mathcal{S}_G \times \mathcal{A} \rightarrow \mathbb{R}$ and $V_G: \mathcal{S}_G \rightarrow \mathbb{R}$ denote the optimal Q and optimal value functions in the ground \ac{MDP}.
\edefn

\bdefn{$Q_A$, $V_A$}
Let $Q_A: \mathcal{S}_A \times \mathcal{A} \rightarrow \mathbb{R}$ and $V_A: \mathcal{S}_A \rightarrow \mathbb{R}$  stand for the optimal Q and optimal value functions in the abstract \ac{MDP}.
\edefn
...
\begin{equation}
\forall_{s \in \mathcal{S}_G}: | V_G^{\pi^*_G}(s) - V_G^{\pi^*_{A}}(s) | \leq poly(\epsilon).
\end{equation}
\end{thm}
...
\forall_{a} \left|\frac{e^{Q^*(s_1,a)}}{\sum_b e^{Q^*(s_1,b)}} - \frac{e^{Q^*(s_2,a)}}{\sum_b e^{Q^*(s_2,b)}}\right| \leq \epsilon
\label{eq:phi_bolt}
\end{multline}
...
\begin{equation}
\forall_{s \in S_M} : | V^{\pi^*_G}(s) - V^{\pi^*_{A}}(s) | \leq \frac{2\epsilon\left(\frac{|\mathcal{A}|}{1-\gamma} + k\epsilon + k\right)}{(1-\gamma)^2}
\end{equation}
...
We consider a multinomial distribution over $Q^*$ for similar reasons as the Boltzmann. We let $\phi_{mult}$ define a type of abstraction that, for fixed $\epsilon$ satisfies.
\begin{multline}
...
\dnote{Bounds for multi and bolt suspisiously similar. Double check math}
