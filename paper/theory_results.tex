...
\begin{lma}
\label{lma:Q*}
When a $\ep_{Q^*,\varepsilon}$ type creates $\mcS_A$:
...
\end{align*}
$s_G$ is arbitrary so we conclude Equation \ref{eq:clm1Induct}. As $T \rightarrow \infty$, $\sum_{t=0}^{T-1} \varepsilon \gamma^t \rightarrow \frac{\varepsilon}{1-\gamma}$ by the sum of infinite geometric series and $Q_T \rightarrow Q_A$. Thus, Equation \ref{eq:clm1Induct} gives Claim \ref{clm:closeQs}.
...
\label{eq:bolt_qs}
\end{equation}
Consequently, we can consider $\epB$ as a of the $\ep_{Q^*,B}$ type, where $B = \varepsilon \left(\frac{|\mathcal{A}|}{1-\gamma} + k\varepsilon + k \right)$. Lemma~\ref{lma:bolt_lemma} then follows from Lemma~\ref{lma:Q*}.
\qed
