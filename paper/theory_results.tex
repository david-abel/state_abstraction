...
We prove this theorem in the following sections by proving bounds on the error of each individual approximate state-aggregation function type.
...
\begin{lma}
\label{lma:Q*}
When $\ep_{Q^*,\varepsilon}$ is used to create the abstract state space $\mcS_A$:
\begin{equation}
...
Thus,
\begin{equation}
\left| Q_{1}(s_T, a) - Q_G(s_G,a) \right| \leq \sum_{t=0}^{1}\varepsilon \gamma^t.
\end{equation}
%
% Inductive Case
...
\end{align*}
Since $s_G$ is arbitrary, we conclude Equation \ref{eq:clm1Induct}.

As $T \rightarrow \infty$, $\sum_{t=0}^{T-1} \varepsilon \gamma^t \rightarrow \frac{\varepsilon}{1-\gamma}$ by the sum of infinite geometric series and $Q_T \rightarrow Q_A$. Thus, Equation \ref{eq:clm1Induct} allows us to conclude Claim \ref{clm:closeQs}.
...
\begin{lma}
\label{lma:model}
When $\mcS_A$ is created using a function of the $\ep_{model,\varepsilon}$ type:
\begin{equation}
...
\begin{align}
\left|\frac{1 + Q_G(s_1,a) + \delta_1}{\sum_j e^{Q_G(s_1,a_j)}} - \frac{1 + Q_G(s_2,a) + \delta_2}{\sum_j e^{Q_G(s_1,a_j)} \pm k\varepsilon}\right| \leq \varepsilon \label{eq:bolt_with_approx}
\end{align}
Either the $1 + x + \delta$ approximation is maximal or minimal in Equation~\ref{eq:bolt_with_approx}. That is, either $\sum_j e^{Q_G(s_1,a_j)} + k\varepsilon$, or $\sum_j e^{Q_G(s_1,a_j)} - k\varepsilon$.

First suppose the former. It follows by algebra that:
\begin{equation}
-\varepsilon \leq \frac{1 + Q_G(s_1,a) + \delta_1}{\sum_j e^{Q_G(s_1,a_j)}} - \frac{1 + Q_G(s_2,a) + \delta_2}{\sum_j e^{Q_G(s_1,a_j)} + k\varepsilon} \leq \varepsilon
\end{equation}
...
In the minimal case, it follows that:
\begin{equation}
-\varepsilon \leq \frac{1 + Q_G(s_1,a) + \delta_1}{\sum_j e^{Q_G(s_1,a_j)}} - \frac{1 + Q_G(s_2,a) + \delta_2}{\sum_j e^{Q_G(s_1,a_j)} - k\varepsilon} \leq \varepsilon
\end{equation}

By similar algebra to the maximal case:
\begin{multline*}
-\varepsilon \left(-k\varepsilon + \sum_j e^{Q_G(s_1,a_j)}\right) - \delta_1 + \delta_2 \leq \\
-k\varepsilon\left(\frac{1+Q_G(s_1,a) + \delta_1}{\sum_j e^{Q_G(s_1,a_j)}}\right) + Q_G(s_1,a) - Q_G(s_2,a) \leq \\
\varepsilon \left(k\varepsilon + \sum_j e^{Q_G(s_1,a_j)}\right) - \delta_1 + \delta_2
...
From both cases, it follows that:
\begin{equation}
