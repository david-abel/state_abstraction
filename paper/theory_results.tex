...
\label{eq:bolt_with_approx}
\end{align*}
There are two cases of interest - either the $1 + x + \delta$ approximation is maximal or minimal in Equation~\ref{eq:bolt_with_approx}. That is, we either have $\sum_j e^{Q_G(s_1,a_j)} + k\varepsilon$, or $\sum_j e^{Q_G(s_1,a_j)} - k\varepsilon$.
