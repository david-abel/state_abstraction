...
% Q* Lemma
We consider an approximate version of the $Q^*$-irrelevance abstraction. An approximate $Q$ function has the same form as Equation~\ref{eq:phi_f}:
\begin{equation}
...
\begin{equation}
\forall_{s \in \mcS_G}: | V^{\pi_G^*}(s) - V^{\pi_{GA}}(s) | \leq \frac{2\epsilon}{(1-\gamma)^2}.
\end{equation}
\end{lma}
...
Then:
\begin{equation*}
\leq \gamma\sum_{t=0}^{T-2} \epsilon \gamma^t + \sum_{g \in X(s_A)}\left[ w(g)\ Q_G(g,a)\right]
\end{equation*}
...
Inspired by the model-irrelevance abstraction, we consider the approximate case. We let $\epM$ define a type of abstraction that, for fixed $\epsilon$, satisfies:
\begin{multline}
...
\begin{equation}
\forall_{s \in \mcS_G} : | V_G(s) - V^{\pi_{GA}}(s) | \leq \frac{2\epsilon + 2\gamma((|\mcS_G|-1)\epsilon)}{(1-\gamma)^3}
\end{equation}
\end{lma}
...
\enote{Do we need to state what Q_max is and that it is upper bounded by 1/(1-gamma) or is this common knowledge?}
