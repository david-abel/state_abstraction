...
\end{equation*}
where $s_A \in \mcS_A$ and $s_1, s_2 \in G(s_A)$. Since difference of rewards is bounded by $\varepsilon$:
% Removed for space
%\begin{multline*}
...
\end{align*}
Since \textsc{QMax} $= \frac{\textsc{RMax}}{1-\gamma}$, and we defined $\textsc{RMax} = 1$:
\begin{equation*}
...
We also assume that the difference in normalizing terms is bounded by some non-negative constant, $k \in \mathbb{R}$, of $\varepsilon$:
\begin{equation}
\left |\sum_i Q_G(s_1,a_i) - \sum_j Q_G(s_2,a_j) \right | \leq k \times \varepsilon.
\end{equation}
