...
% Theorem 1
\begin{thm}
There exist at least four classes of approximate state-aggregation functions, $\ep_{Q^*}$, $\ep_{\text{model}}$, $\ep_{\text{bolt}}$ and $\ep_{\text{mult}}$, for which the optimal policy in the abstract \ac{MDP}, applied to the ground \ac{MDP}, has suboptimality bounded polynomially in $1/\epsilon$:
\begin{equation}
...
\end{equation*}
zzz again, doesn't say what it is less than or equal to
...
\end{cases}
\end{equation}
zzz inconsistent notation for cases. I like the ifs, but that's not how it appeared earlier.
...
\end{multline*}
Therefore,
zzz more dangling inequalities below.
\begin{align*}
...
zzz so, these Qmodel things are formatted better. use the same formatting earlier?
...
zzz again... this dangling comparison operator thing is highly non-standard.
\begin{multline*}
...
\begin{align*}
 \leq \epsilon + \gamma Q_{max} \sum_{s_A \in \mcS_A} \epsilon \leq \epsilon + \gamma|\mcS_G|\epsilon Q_{max}.
\end{align*}
zzz similarly, Qmax needs to be formatting differently.
Applying 0-1 reward normalization and algebra:
\begin{equation*}
 \leq \frac{\epsilon + \gamma(|\mcS_G| - 1) \epsilon}{1-\gamma}.
\end{equation*}
\enote{Do we need to state what Q max is and that it is upper bounded by 1 over 1 minus gamma or is this common knowledge? zzz not common knowledge, no.}
...
Here, we introduce the approximate abstraction family, $\phi_{bolt}$, which aggregates states with similar Boltzmann distributions on optimal $Q$ values as equivalent. This family of abstractions is appealing as the Boltzmman distribution over $Q$ values has been of interest in the RL literature due to its natural balancing of exploration and exploitation~\cite{sutton1998reinforcement}.
...
\label{eq:bolt_qs}
\end{equation}
Consequently, we can consider $\widetilde{\phi}_{bolt}$ as a variation of $\epQ$, where the difference in $Q$ values is bounded by $\epsilon \left(\frac{|\mathcal{A}|}{1-\gamma} + k\epsilon + k \right)$. Lemma~\ref{lma:bolt_lemma} then follows directly from Lemma~\ref{lma:Q*}.
...
zzz a bit more info is needed. what advantages does it have over qboltz. if none, why do we need it?
\bdefn{$\ep_{mult}$}
...
\end{equation}
zzz why is that ok? why is it useful? (overall, I think we should take a slightly more pedagogical perspective. we're not writing programs, we're writing explanations.
...
{\bf Proof Sketch:} The proof follows an identical strategy to that of Lemma~\ref{lma:bolt_lemma}, except the approximation for $e^x$ is not needed.
