...
\end{multline*}
%&\leq \gamma\sum_{t=0}^{T-2} \epsilon \gamma^t + \sum_{g \in X(s_A)}\omega(g)\left[ R_0(g,a) + \gamma \sum_{g' \in \mathcal{S}_G} T_0(g,a,g') \max_{a'}Q_0      \right]\\
Then,
\begin{equation*}
...
Note that as $t \rightarrow \infty$, $\sum_{i=0}^{t}\gamma^i \frac{2\epsilon}{1-\gamma} \rightarrow \frac{2\epsilon}{(1-\gamma)^2}$ by the sum of infinite geometric series and $\pi_{A,t}(s) \rightarrow \pi_{GA}$.
...
\end{multline}
\edefn
\dnote{Possible explanation of what Boltzmann does in language, here}
...
We consider approximate abstractions derived from a multinomial distribution over $Q^*$ for similar reasons to our motivation in considering the Boltzmann distribution. \dnote{Relationship between multi and bolts, why need this one, argument: it's simpler! simpler is good}
\bdefn{$\epMu$}
...
\end{multline}
\edefn
\dnote{Some explanation of what this one does}
...
\end{equation}
\dnote{Some explanation of why it's okay to assume this. Answer: Q magnitudes can be artbirarily off, making the denominators similar lets us say, Hm. These q magnitudes are roughly in the same ballpark.}
