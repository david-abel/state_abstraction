...
\icmltitlerunning{Suboptimality Bounds for Approximate State Abstraction}
...
\twocolumn[
\icmltitle{Suboptimality Bounds for Approximate State Abstraction}
...
\enote{Say more about the weighting (that it sums to 1 but that's all we assume)}
...
\defn{A $Q^*$-irrelevance abstraction, $\phi_{Q^*}$, is one such that:
\begin{equation}
\phi_{Q^*}(s_1) = \phi_{Q^*}(s_2) \rightarrow \forall a\ \left(Q^*(s_1,a) = Q^*(s_2,a)\right).
\end{equation}
}

\defn{A model-irrelevance abstraction, $\phi_{model}$, is one such that:
\begin{multline}
...
% Theorem 1
{\bf Theorem 1:} There exist at least four classes of approximate state-aggregation functions, $\ep_{Q^*}$, $\ep_{\text{model}}$, $\ep_{\text{bolt}}$ and $\ep_{\text{mult}}$ for which the optimal policy in the abstract \ac{MDP}, applied to the ground \ac{MDP}, has suboptimality bounded polynomially in $\epsilon$:
\begin{equation}
...
% zzz wy not \begin{lemma} ? you know, to get the number right automatically?
{\bf Lemma 1:} When $\ep_{Q^*}$ is used to create the abstract state space $\mathcal{S}_A$:
\begin{equation}
...
%Claim 1 zzz \begin{claim} ?
\textbf{Claim 1 -- Q-Values in the abstract \ac{MDP} closely resemble those in the ground \ac{MDP}:}
\begin{equation}
\label{eq:Q*Claim1}
...
Since the Q values of ground states grouped under $\epM$ are strictly less than $B$, we can understand $\epM$ as a type of $\epQ$ with $\epsilon = B$. Applying Lemma 1 yields Equation \ref{eq:lemma2}.
...
\enote{Note somewhere why we selected these MDPs -- b/c a wide variety}
...
\dnote{One thing we want to add is a description of how diverse these domains are!}
