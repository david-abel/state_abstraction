...
That is, when $\phi_f$ aggregates states, all aggregated states the aggregated states have values of $f$ within $\epsilon$ of one another for all actions.

Note that for any $f$, there are many such functions that satisfy this criteria.
...
\subsection{Similarity of Value Yields Similar Behaviour}
\enote{Proof that if values in the abstract and ground are similar then behavior is similar. State that for remainder of lemmas that we prove antecedent of this statement.}

% Subsection: Optimal Q Function
\subsection{Optimal Q Function: $\ep_{Q^*}$}

% Q* Lemma
Recall that $Q^*$ is a function on $\mathcal{S}_G \times \mathcal{A}$ that satisfies \enote{$Q^*$ definition}. Consequently,  as per Equation $\ref{eq:phi_f}$, $\ep_{Q^*}$ is a type of abstraction that for a fixed $\epsilon$ satisfies:
\begin{equation}
