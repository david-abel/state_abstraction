...
&\text{Thus,}\\
&Q_1(s,a) \left| Q_{T}(s_A, a) - Q_0(s_G,a) \right| \leq \gamma\sum_{t=0}^{1}\epsilon \gamma^t
\end{align*}
...
&Q_T(s_A, a) =\sum_{g \in X}w(g)\left[ R_0(g,a) + \gamma \sum_{g' \in \mathcal{S}_G} T_0(g,a,g') \max_{a'} Q_{T-2}(g', a')      \right]\\
...
By Equation \ref{eq:Q*Claim1} and the definition of $a^*_G$ and $a^*_A$ we know that 
\begin{align}
...
% PUT INTO RESULTS One interesting fact about UpWorld is that, if $M$ the width of the grid, is defined to be an arbitrary function of $N$, say $k^N$ for some $k \in \mathbb{Z}$, then optimality preserving abstractions can arbitrarily reduce the sample complexity of the problem.
