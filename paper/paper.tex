...
\begin{equation}
{M_G}' =  \langle \mathcal{S}_G, \mathcal{A}, \mathcal{R}_G', \mathcal{T}_G', \gamma \rangle
\end{equation}
...
It is easily verifiable that ${\mcT_G}'$ induces a valid probability distribution.
...
Similarly, the transitions of states in ${M_G}'$ that are aggregated in $M_A$ are identical:
\begin{align*}
\forall{s1_G, s2_G, {s_G}' \in \mcS_G, a}\\
\left[ \epM(s1_G) = \epM(s2_G) \rightarrow {\mcT_G}'(s1_G, a, {s_G}') = {\mcT_G}'(s2_G, a, {s_G}') \right]
\end{align*}

Now consider applying to $M_G$' the state aggregation scheme, $\phi_m$, which satisfies the following:
\begin{align*}
\forall{a, s_G \in \mcS_G}  [&{\mcR_G}'(s1_G, a) = {\mcR_G}'(s2_G, a) \wedge \\
&{\mcT_G}'(s1_G, a, {s_G}') = {\mcT_G}'(s2_G, a, {s_G}') ]  \rightarrow \\
\phi_m(s1_G) = \phi_m(s2_G)
\end{align*}

$\phi_m$ is of the $\pM$ type and when applied to ${M_G}'$ results in $M_A$.

 \cite{li2006towards} demonstrate that abstraction schemes of the $\pM$ type maintain Q values so we know that $\forall{s_G \in \mcS} Q_{G'}(s_G) = Q_{A}(s_G)$.
  
Knowing that the Q values in $M_A$ are identical to those in ${M_G}'$, we now demonstrate that those of $M_G$ are boundedly close to those of ${M_G}'$ and then conclude by applying Lemma 1 (which states similar Q values in the abstract and ground MDP results in near optimal behavior of the optimal abstract policy applied to the ground MDP).

Let the maximum upper bound between optimal behavior in the noisy and ground MDP be called $B$.
